...
summarizeFamilies(nlsy_ped, famID = "famID")$family_summary |> arrange(desc(count))
...
This is cool, would like to try to run 
```{r}
fam_820 <- fam_820 %>%
  rename(personID = "ID")
...
ggsave('plot820.PNG', plot_820, bg = "transparent")
...
1) Wake theme, no transparent background though, so you can see it. 
```{r}
...
In the final version on the poster, the ID labels were too much, so I make them blank instead. 
```{r}
